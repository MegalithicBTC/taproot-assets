...
func randEncodedAddress(t *testing.T, net *address.ChainParams, groupPubKey,
	...
	newAddr, err := randAddress(
		t, net, address.V0, groupPubKey, sibling, nil, assetType,
		addrOpts...,
	)
	if err != nil {
		return nil, "", err
...
func TestNewAddress(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			f: func() (*address.Tap, error) {
				return randAddress(
					t, &address.TestNet3Tap, address.V0,
		...
		},
		{
			...
			f: func() (*address.Tap, error) {
				return randAddress(
					t, &address.MainNetTap, address.V0,
					true, false, nil, asset.Collectible,
					address.WithAssetVersion(asset.V1),
				)
			},
			err: nil,
		...
		},
		{
			...
			f: func() (*address.Tap, error) {
				return randAddress(
					t, &address.MainNetTap, address.V0,
					true, true, nil, asset.Collectible,
				)
			},
			err: nil,
		},
		{
			...
			f: func() (*address.Tap, error) {
				...
				return randAddress(
					t, &address.TestNet3Tap, address.V0,
					false, false, &zeroAmt, asset.Normal,
				)
			},
		...
		},
		{
			...
			f: func() (*address.Tap, error) {
				...
				return randAddress(
					t, &address.TestNet3Tap, address.V0,
					false, false, &badAmt, asset.Collectible,
				)
			},
		...
		},
		{
			...
			f: func() (*address.Tap, error) {
				return randAddress(
					...
					address.Version(123), false, false, nil,
					asset.Normal,
				)
			},
	...
	assertAddressEncoding := func(comment string, a *address.Tap) {
		...
		testVectors.ValidTestCases = append(
			testVectors.ValidTestCases, &addressmock.ValidTestCase{
				Address:  addressmock.NewTestFromAddress(t, a),
				Expected: addr,
				Comment:  comment,
			},
		)
	...
	testCases := []struct {
		...
		},
		{
			...
			f: func() (*address.Tap, string, error) {
				_, err := randAddress(
					t, &address.TestNet3Tap,
					address.Version(255), false, true, nil,
					asset.Collectible,
				)
				return nil, "", err
			},
